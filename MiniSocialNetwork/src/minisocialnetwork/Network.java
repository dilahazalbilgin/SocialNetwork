...
public class Network {
    ...
    public void viewActivityFeed(int userId) {
        ...
        System.out.println(user.getName() + "'s Activity Feed:");

        List<ActivityFeed> allActivities = new ArrayList<>();

        allActivities.addAll(user.getActivityFeed());

        for (UserManagement friend : user.getFriends()) {
            allActivities.addAll(friend.getActivityFeed());
        }

        allActivities.sort((a, b) -> Long.compare(b.getTimestamp(), a.getTimestamp()));

        for (ActivityFeed activity : allActivities) {
            System.out.println("[" + new Date(activity.getTimestamp()) + "] " + activity.getDescription());
        }
    }
    ...
    public List<UserManagement> recommendFriends(int userId) {
        ...
        if (user == null) {
            System.out.println("User doesn' exist.");
